    ...
    F: FnMut(WhisperLoadPhase, PhaseState),
{
    ...
    // Determine the actual model source
    ...
    let (final_model_id, is_remote) = if is_hf_id {
        ...
        // Path doesn't exist and isn't an HF ID; use default
    ...
    // Token IDs differ between .en and multilingual models, so we MUST look them up
    let sot = tokenizer
    ...
    let language_token = if is_en_only {
        ...
        // Don't default to English! None = auto-detect in inference.rs
    };
